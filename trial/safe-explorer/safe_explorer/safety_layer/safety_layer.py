...
class SafetyLayer:
    ...
    def get_safe_action(self, state, action, constraints):
        ...
        state = torch.DoubleTensor(state).detach()
        action = torch.DoubleTensor(action).detach()
        constraints = torch.DoubleTensor(constraints).detach()

        g = [model.forward(state).detach() for model in self.models]
        # Calculate lagrange multipliers
        multipliers = [torch.clip((torch.dot(gi, action) + ci) / torch.dot(gi, gi), min=0).detach() for gi, ci in zip(g, constraints)]

        # Calculate correction; scale correction to be more aggressive
        max_multiplier_index = np.argmax([multiplier.item() for multiplier in multipliers])
        safe_action = action - multipliers[max_multiplier_index] * g[max_multiplier_index] * self.correction_scale
        safe_action = safe_action.detach().numpy()  # Ensure detachment before converting to NumPy array
